...
function ReviewRow({ icon, label, value }: { icon: React.ElementType, label: string, value: React.ReactNode }) {
    const Icon = icon;
    return (
        <div className="flex items-start justify-between py-2 sm:grid sm:grid-cols-3 sm:gap-4">
            <div className="flex items-center gap-2 text-sm text-muted-foreground sm:col-span-1">
                <Icon className="h-4 w-4" />
                <span>{label}</span>
            </div>
            <div className="w-2/3 text-sm font-medium break-words text-right sm:text-left sm:col-span-2">
                {value}
            </div>
        </div>
    )
